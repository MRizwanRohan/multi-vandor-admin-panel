...
async function loadData() {
  ...
  try {
    ...
    const result = await analyticsService.getTopVendors(params)
    vendors.value = Array.isArray(result) ? result : []
  } catch (e: any) {
    error.value = e?.response?.data?.message || 'Failed to load top vendors'
